...
function calculateBasePrice(formData) {
  ...
  let totalPrice = zoneBase + volumePrice;
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (pricing.surcharges.noAscensor > 0) {
        doc.text(`Recargo sin ascensor: $${pricing.surcharges.noAscensor.toLocaleString('es-CL')}`, 40, priceY + 65);
      }
      if (pricing.surcharges.delicado > 0) {
        doc.text(`Muebles delicados: $${pricing.surcharges.delicado.toLocaleString('es-CL')}`, 40, priceY + 80);
      }
      if (pricing.surcharges.estacionamiento > 0) {
        doc.text(`Estacionamiento lejano: $${pricing.surcharges.estacionamiento.toLocaleString('es-CL')}`, 40, priceY + 95);
      }
...
export async function processQuotation(formData) {
  try {
    ...
    // Calcular precio
    const pricing = calculateBasePrice(formData);
